...
#===进程级===
ps -aux |grep mongo | grep -v grep|awk '{print $2}' #查找进程pid，比如mongo
prlimit --pid 26254 --nofile=524288:524288  #调整 进程(PID 26254)的最大打开文件数（nofile）软/硬限制为 524288。
cat /proc/26254/limits | grep "Max open files" #检查，若还是没变，说明：权限不足、系统全局限制没放开、进程内部锁死、systemd 限制未改 可以结合前述方法（如 systemd LimitNOFILE、limits.conf、fs.file-max）一起调整。
